...
class CheckDocsTest < Minitest::Test
  ...
  def test_missing_language_entry_fails
    with_fixture(overrides: { "README.md" => "# Project\n" }) do |root|
      result = run_checker(root)
      refute result[:status].success?
      assert_includes result[:output], "Missing Chinese entry link"
    end
  end

  def test_legacy_organization_reference_fails
    legacy_name = ["Agent", "Commons\n"].join
    with_fixture(overrides: { "CHANGELOG.md" => legacy_name }) do |root|
      result = run_checker(root)
      refute result[:status].success?
      assert_includes result[:output], "Legacy organization reference"
    end
  end
  ...
  def with_fixture(remove: [], overrides: {})
    Dir.mktmpdir("curl-builder-docs-") do |root|
